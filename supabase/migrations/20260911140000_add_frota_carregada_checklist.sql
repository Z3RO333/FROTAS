alter table public.checklists_frota add column if not exists frota_carregada boolean;

CREATE OR REPLACE FUNCTION public.criar_checklist_atomico_v2(p_checklist jsonb, p_itens jsonb DEFAULT '[]'::jsonb, p_pendencias jsonb DEFAULT '[]'::jsonb, p_km_history jsonb DEFAULT NULL::jsonb, p_abastecimento jsonb DEFAULT NULL::jsonb, p_vehicle_summary jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  insert into public.checklists_frota (
    ...
    nullif(p_checklist->>'frota_carregada', '')::boolean
