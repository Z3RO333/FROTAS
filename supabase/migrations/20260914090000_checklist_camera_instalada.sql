-- Pergunta "o veículo possui câmera instalada?" no checklist, com foto
-- obrigatória da câmera quando a resposta é sim. Novo passo no wizard do
-- motorista, entre os itens críticos e o hodômetro.

begin;

alter table public.checklists_frota
  add column if not exists possui_camera boolean,
  add column if not exists foto_camera_url text;

-- Mesma assinatura de criar_checklist_atomico_v2 (jsonb), só passa a ler
-- possui_camera/foto_camera_url de dentro de p_checklist e gravar nas
-- colunas novas.
