-- Corrige duas lacunas achadas ao investigar a frota 232 (id 203): em
-- 10/09 uma leitura de OCR falhou, o motorista digitou por cima e o KM
-- caiu ~242.000 de uma vez (287.780 -> 45.886). Isso passou porque:
--
--   1. trg_valida_teto_km_por_turno só bloqueava saltos GRANDES PRA CIMA
--      (v_delta > 50000). Quedas grandes não tinham teto nenhum — só
--      exigiam texto livre no campo de justificativa (qualquer texto).
--   2. criar_checklist_atomico_v2 nunca persistia justificativa_km no
--      histórico — a migration 20260902100000 tentou corrigir isso mas,
--      segundo a migration 20260909120000, nunca chegou a ser aplicada
--      neste banco. O texto que autorizava a divergência não deixava
--      rastro nenhum.
--
-- Esta migration: (a) torna o teto do trigger simétrico (abs(delta) >
-- 50000, pra cima ou pra baixo) e (b) volta a persistir justificativa_km
-- no histórico, recriando a função a partir da definição atualmente viva
-- em produção (com possui_camera/foto_camera_url já presentes) para não
-- reverter nenhum ajuste feito depois de 20260902100000.
...
begin
  ...
  insert into public.checklists_frota (
    ...
    foto_km_url, possui_camera, foto_camera_url,
    status_geral, observacao_original, observacao_corrigida_ia
  ) values (
    ...
    nullif(p_checklist->>'possui_camera', '')::boolean,
    p_checklist->>'foto_camera_url',
