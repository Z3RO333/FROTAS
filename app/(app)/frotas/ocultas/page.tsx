...
function pageHref(searchParams: Record<string, string | undefined>, page: number) {
  ...
  return `/frotas/ocultas?${next.toString()}`;
}

export default async function FrotasOcultasPage({
  searchParams,
}: {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-slate-950">Frotas ocultas</h1>
          <p className="text-sm text-muted-foreground">
            {total} frota(s) ocultada(s) — não vendidas, apenas retiradas da listagem principal. Reative
            pela edição de cada frota.
          </p>
        </div>
      ...
      <FrotasFilters modelos={modelos} setores={setores} cds={cds} basePath="/frotas/ocultas" />
